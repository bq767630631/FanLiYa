...
- (void)awakeFromNib {
    [super awakeFromNib];
     CGFloat imageWH = (SCREEN_WIDTH - 5 *3 ) / 2;
    self.imageHCons.constant = imageWH;
    ViewBorderRadius(self.imageView, 5, RGBColor(241, 241, 241));
}


- (void)setInfo:(id)info{
    SearchResulGoodInfo *goodInfo = info;
    NSString *picImag = goodInfo.isTuiJianGood?goodInfo.img:goodInfo.pic;
    [self.imageView setDefultPlaceholderWithFullPath:picImag];
    ...
    self.soldNum.text = goodInfo.isTuiJianGood ? @"":[NSString stringWithFormat:@"%@人购买",goodInfo.sold_num];
